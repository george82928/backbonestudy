$(function() {
	...
	var StudentView = Backbone.View.extend({
		...
		editing: function(e) {
			$(e.currenTarget).removeClass('show').addClass('editing').find('input,select').focus();
		},
		...
		setValue: function() {
			...
			$(this.el).find('input,select').each(function() {
				var $scurele = $(this);
				$scurele.val(model.get($curele.attr('name')));
			});
		}
	...
	var stuAppView = Backbone.View.extend({
		...
		addData: function(stu) {
			...
			var stuView = new StudenView({model:stu});
